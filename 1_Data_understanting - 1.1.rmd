...
# Adicionar a coluna payment_type do dataset payment ao data frame data_customer
```{r}
# Agregar os dados de payment para garantir apenas uma linha por order_id
payment_aggregated <- payment %>%
                        group_by(order_id) %>%
                        summarise(payment_type = first(payment_type))

# Realizar a junção entre data_customer e payment_aggregated
data_customer <- data_customer %>%
                  inner_join(payment_aggregated, by = "order_id")
```

# Criar a coluna 'credit' com base na coluna 'payment_type'
```{r}
data_customer$credit <- as.integer(data_customer$payment_type == "credit_card")
data_customer$boleto <- as.integer(data_customer$payment_type == "boleto")
data_customer$debit <- as.integer(data_customer$payment_type == "debit_card")
data_customer$voucher <- as.integer(data_customer$payment_type == "voucher")
```
