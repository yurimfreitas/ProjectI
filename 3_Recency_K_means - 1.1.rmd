...
data_customer_segmentation <- data_customer_cutoff %>%
  ...
  summarise(count_orders = n(),
            max_purchase = max(order_purchase_timestamp),
  ...
  select(customer_unique_id, target, total_paid, count_orders, credit, boleto, debit, voucher, max_purchase)
...
```{r}
customer_segmentation_recency_num <- data_customer_segmentation[,10]
