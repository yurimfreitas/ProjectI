...
# Count canceled
print(sum(data_customer$order_status == "canceled"))
# Excluding  canceled
data_customer <- data_customer[data_customer$order_status != "canceled", ]
# Checking if there are still canceled
print(sum(data_customer$order_status == "canceled"))
```
...
# looking order_purchase_timestamp
```{r}
#  Where is NA
print(subset(data_customer, is.na(order_purchase_timestamp)))
...
Q1 <- quantile(data_customer$total_price, 0.25)
Q3 <- quantile(data_customer$total_price, 0.75)
IQR <- Q3 - Q1
...
outliers <- data_customer$total_price < limite_inferior | data_customer$total_price > limite_superior
