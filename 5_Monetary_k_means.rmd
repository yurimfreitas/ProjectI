...
counts <- data_customer_segmentation %>%
  ...
  summarise(
    count = n(),
    min_total_paid = min(total_paid),
    max_total_paid = max(total_paid)
  )
...
# Save df in a RData to be used
```{r}
# Salvar como RData
write.csv(data_customer_segmentation, "data_customer_segmentation.csv", row.names = TRUE)
```
