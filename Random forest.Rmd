...
#looking data
```{r}
str(data_customer_segmentation)
```
# Exclude customer_unique_id
```{r}
...
```{r}
data_customer_segmentation$target <- as.factor(data_customer_segmentation$target)
```
...
```{r}
set.seed(123) 
trainIndex <- createDataPartition(data_customer_segmentation$target, p = .8, 
                                  list = FALSE, 
                                  times = 1)
...
# Verifique os níveis da variável target
levels(data_train$target)

# Aplique make.names() para os níveis
data_train$target <- make.names(data_train$target)

# Treine o modelo novamenteZ 
seed <- 123
set.seed(seed)
